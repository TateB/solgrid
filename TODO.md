...
- [ ] `security/reentrancy` — detect state changes after external calls (CEI violation) — **semantic, complex**
- [ ] `security/avoid-selfdestruct` — flag `selfdestruct` (deprecated in Solidity 0.8.18+)
- [ ] `security/compiler-version` — require specific or minimum compiler version (+ auto-fix suggestion)
- [ ] `security/not-rely-on-block-hash` — avoid `blockhash()` for randomness
- [ ] `security/not-rely-on-time` — avoid `block.timestamp` / `now` for critical logic
- [ ] `security/multiple-sends` — flag multiple `send()` calls in one function
- [ ] `security/payable-fallback` — require payable on fallback/receive (+ auto-fix suggestion)
- [ ] `security/no-delegatecall-in-loop` — flag `delegatecall` inside loops
- [ ] `security/unchecked-transfer` — flag ERC20 transfer without return check
- [ ] `security/msg-value-in-loop` — flag `msg.value` access inside loops
- [ ] `security/arbitrary-send-eth` — flag send/transfer/call to user-controlled address
- [ ] `security/uninitialized-storage` — detect uninitialized storage pointers
- [ ] `security/divide-before-multiply` — detect precision loss from division before multiplication

### Chunk 6: Best Practices Rules Expansion (19 rules)
_Add remaining best practices rules from ARCHITECTURE.md._

- [ ] `best-practices/no-unused-vars` — detect unused local variables **(semantic)**
- [ ] `best-practices/no-unused-imports` — detect unused imports **(semantic)**
- [ ] `best-practices/no-unused-state` — detect unused state variables **(semantic)**
- [ ] `best-practices/code-complexity` — flag functions exceeding cyclomatic complexity threshold
- [ ] `best-practices/function-max-lines` — flag functions exceeding line count (default: 50)
- [ ] `best-practices/max-states-count` — flag contracts with too many state variables (default: 15)
- [ ] `best-practices/one-contract-per-file` — enforce one contract/interface/library per file
- [ ] `best-practices/no-global-import` — disallow `import "file.sol"` (+ auto-fix suggestion)
- [ ] `best-practices/constructor-syntax` — use `constructor` keyword (+ auto-fix safe)
- [ ] `best-practices/use-natspec` — require NatSpec on public/external functions
- [ ] `best-practices/natspec-params` — NatSpec @param for every function parameter
- [ ] `best-practices/natspec-returns` — NatSpec @return for every return value
- [ ] `best-practices/reason-string` — require reason strings in require/revert
- [ ] `best-practices/custom-errors` — prefer custom errors over require with string (+ suggestion fix)
- [ ] `best-practices/no-floating-pragma` — disallow floating pragma (+ safe fix)
- [ ] `best-practices/imports-on-top` — all imports must be at the top
- [ ] `best-practices/visibility-modifier-order` — enforce Solidity style guide order (+ safe fix)
- [ ] `best-practices/no-unused-error` — detect declared but unused custom errors (+ suggestion fix)
- [ ] `best-practices/no-unused-event` — detect declared but unused events (+ suggestion fix)

### Chunk 7: Naming Rules Expansion (13 rules)
_Add remaining naming rules from ARCHITECTURE.md._

- [ ] `naming/interface-starts-with-i` — interface names must start with `I`
- [ ] `naming/library-name-capwords` — libraries must use CapWords
- [ ] `naming/struct-name-capwords` — structs must use CapWords
- [ ] `naming/enum-name-capwords` — enums must use CapWords
- [ ] `naming/event-name-capwords` — events must use CapWords
- [ ] `naming/error-name-capwords` — custom errors must use CapWords
- [ ] `naming/param-name-mixedcase` — function parameters must use mixedCase
- [ ] `naming/var-name-mixedcase` — local variables must use mixedCase
- [ ] `naming/immutable-name-snakecase` — immutable variables must use UPPER_SNAKE_CASE
- [ ] `naming/private-vars-underscore` — private/internal state vars must start with `_`
- [ ] `naming/modifier-name-mixedcase` — modifiers must use mixedCase
- [ ] `naming/type-name-capwords` — user-defined value types must use CapWords
- [ ] `naming/foundry-test-functions` — test functions must match test/testFuzz/testFail patterns
...
| 5. Security Expansion | TODO | +13 | Remaining security rules |
| 6. Best Practices Expansion | TODO | +19 | Remaining best practices rules |
| 7. Naming Expansion | TODO | +13 | Remaining naming rules |
...
**Current state:** 12/90 rules implemented, working CLI, all tests passing.
