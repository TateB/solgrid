...
- [ ] Extend semantic/interprocedural detector depth beyond the shipped same-file, inherited-helper, transitive imported-wrapper-chain, contract-typed helper-wrapper propagation through uniquely resolved member/indexed bases, collapsed helper-returning call targets, non-unique helper contracts with a shared propagated result, and common-propagated-result-disambiguated imported wrapper overloads, especially across arbitrary non-unique cross-file call graphs, empty-overlap overload summaries, and non-unique wrapper graphs with no shared propagated sink evidence
