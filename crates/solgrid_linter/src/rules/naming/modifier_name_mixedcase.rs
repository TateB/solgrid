//! Rule: naming/modifier-name-mixedcase
//!
//! Modifiers must use mixedCase (camelCase).
...
static META: RuleMeta = RuleMeta {
    id: "naming/modifier-name-mixedcase",
    name: "modifier-name-mixedcase",
    category: RuleCategory::Naming,
    default_severity: Severity::Warning,
    description: "modifier name should use mixedCase (camelCase)",
    fix_availability: FixAvailability::None,
};

pub struct ModifierNameMixedcaseRule;

impl Rule for ModifierNameMixedcaseRule {
                            ...
                            if let Some(name_ident) = func.header.name {
                                ...
                                if !solgrid_ast::is_camel_case(name) {
                                    ...
                                    diagnostics.push(Diagnostic::new(
                                        META.id,
                                        format!(
                                            "modifier name `{name}` should use mixedCase (camelCase)"
                                        ),
