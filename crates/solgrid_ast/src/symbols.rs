...
pub(crate) fn display_for_type(
    ...
    let mut display = normalize_whitespace(&source[span_to_range(ty.span)]);
...
fn parameter_label(source: &str, param: &VariableDefinition<'_>) -> String {
    normalize_whitespace(&source[span_to_range(param.span)])
}

fn signature_data_for_function(source: &str, func: &ItemFunction<'_>) -> SignatureData {
    let label = normalize_whitespace(&source[span_to_range(func.header.span)]);
    ...
    let return_types: Vec<TypeSpec> = func
        ...
        .map(|param| {
            type_spec_from_ast(
                ...
                &param.ty,
                param.data_location,
                span_to_range(func.header.span).start,
            )
        })
        .collect();
...
fn collect_item(
    ...
    match &item.kind {
        ItemKind::Contract(contract) => {
            ...
            let name_span = span_to_range(contract.name.span);
        ...
        ItemKind::Function(func) => {
            ...
            if let Some(name_ident) = func.header.name {
                ...
                let name_span = span_to_range(name_ident.span);
                table.add_symbol(
                    parent_scope,
                    SymbolDef {
                ...
                let header_start = span_to_range(func.header.span).start;
                table.add_symbol(
                    parent_scope,
                    SymbolDef {
            ...
            for param in func.header.parameters.iter() {
                if let Some(name_ident) = param.name {
                    let name_span = span_to_range(name_ident.span);
                    let param_def_span = span_to_range(param.span);
                    table.add_symbol(
                        func_scope,
                        SymbolDef {
                            name: name_ident.as_str().to_string(),
        ...
        ItemKind::Struct(s) => {
            let name_span = span_to_range(s.name.span);
            let struct_span = span_to_range(item.span);
            ...
            for field in s.fields.iter() {
                if let Some(name_ident) = field.name {
                    let f_name_span = span_to_range(name_ident.span);
                    let f_def_span = span_to_range(field.span);
                    table.add_symbol(
                        struct_scope,
                        SymbolDef {
                            name: name_ident.as_str().to_string(),
        ...
        ItemKind::Enum(e) => {
            let name_span = span_to_range(e.name.span);
            let enum_span = span_to_range(item.span);
            ...
            for variant in e.variants.iter() {
                let v_span = span_to_range(variant.span);
                table.add_symbol(
                    enum_scope,
                    SymbolDef {
        ...
        ItemKind::Udvt(u) => {
            let name_span = span_to_range(u.name.span);
            table.add_symbol(
                parent_scope,
                SymbolDef {
        ...
        ItemKind::Import(import) => {
            ...
            let path_span = span_to_range(import.path.span);
...
fn collect_stmt(table: &mut SymbolTable, scope: ScopeId, source: &str, stmt: &Stmt<'_>) {
    match &stmt.kind {
            ...
            if let Some(name_ident) = var_def.name {
                ...
                table.add_symbol(
                    ...
                    SymbolDef {
                        ...
                        name_span,
                        def_span: span_to_range(stmt.span),
                        scope: None,
                        type_info: Some(type_spec_from_ast(
                            source,
                            &var_def.ty,
        ...
        StmtKind::Block(block) => {
            let block_span = span_to_range(stmt.span);
        ...
        StmtKind::For { init, body, .. } => {
            let for_span = span_to_range(stmt.span);
